...
use self::rule::{
    Element, Env, EnvList, Exp, Fxp, Gxp, Hxp, Ixp, Judgement, Jxp, Kxp, LongExp, Parser, Value,
    ARROW, COMMA, CONS, EQ, EVALTO, FUN, IS, LBRACKET, LESS, LPAREN, MINUS, NUM, PLUS, RBRACKET,
    REC, RPAREN, THAN, TIMES, TRUTH, VAR, VDASH,
};
...
enum Rule {
    ...
    EMatchNil(Derivation, Derivation),
    EMatchCons(Derivation, Derivation),
    BPlus,
    BMinus,
    BTimes,
    BLt,
...
impl std::fmt::Display for Rule {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            Rule::EMatchNil(d0, d1) => write!(f, "E-MatchNil {{{};{}}}", d0, d1),
            Rule::EMatchCons(d0, d1) => write!(f, "E-MatchCons {{{};{}}}", d0, d1),
...
fn eval_longexp(env: &Env, le: &LongExp) -> io::Result<(Derivation, Value)> {
    let (rule, v) = match le {
        ...
        LongExp::LongExp4(_, e0, _, _, _, _, e1, _, x, _, y, _, e2) => {
            ...
            match pop(&v0)? {
                Some((v0_, v1_)) => {
                    let env_ = push_env(&push_env(env, x, &v0_), y, &v1_);
                    let (d1, v1) = eval_exp(&env_, e2)?;
                    (Rule::EMatchCons(d0, d1), v1)
                }
                None => {
                    let (d1, v1) = eval_exp(env, e1)?;
                    (Rule::EMatchNil(d0, d1), v1)
                }
            }
        }
    };
